...
def cli_parser(prog="pca.py"):
    ...
    parser.add_argument('-s', metavar="save_pickle", dest="save_pickle", nargs=1, type=str, help="(optional) Path to save the pickle file for [coordinates, evalues, evectors, covariation_matrix]")
    return parser
...
def main():
    ...
    coordinates, evalues, evectors, covariation_matrix = pca(traj_info, top_info, align_conf, ncpus)

    if args.save_pickle:
        with open(args.save_pickle[0], 'wb') as handle:
            pca_res = [coordinates, evalues, evectors, covariation_matrix]
